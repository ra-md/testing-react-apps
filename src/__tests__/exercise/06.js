...
test('displays the users current location', async () => {
  ...
  expect(screen.getByText(/Latitude/i).textContent).toMatch(`Latitude: ${fakePosition.coords.latitude}`)
  expect(screen.getByText(/Longitude/i).textContent).toMatch(`Longitude: ${fakePosition.coords.longitude}`)
})

test('displays error message when geolocation is not supported', async () => {
  const fakeError = 'Geolocation is not supported or permission denied'
  ...
  act(() => {
    setReturnValue([null, {message: fakeError}])
  })

  expect(screen.queryByLabelText(/loading/i)).not.toBeInTheDocument()
  expect(screen.getByRole('alert')).toHaveTextContent(fakeError)
})
